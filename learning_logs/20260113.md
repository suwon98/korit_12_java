...
## 제네릭
- 클래스, 인터페이스, 메드에 사용되는 자료형을 매개변수화 
할 수 있는 문법으로 다양한 타입에서 재사용이 가능
```java
public class Box<T>{
    // field 선언
    private T value;
...
## 와일드 카드
- 이상의 제네릭 타입을 사용할 때 구체적인 타입을 명시하지 않고
범용적으로 처리하고 싶을 때 사용함. `?` 사용
```java
...
1. List : 순서가 있는 데이터의 집합으로, 중복된 요소를 허락함.
    - ArrayList : 배열 기반의 리스트, element 접근 속도가 바름.
...
- 근데 또 python에서의 attribute의 value입니다.