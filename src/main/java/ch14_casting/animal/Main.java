...
//        animal1.fetch();      // 이게 불가능합니다.
        ...
        // 업캐스팅 # 2
        Animal animal2 = new Dog(); // 이게 이제 말한 A a = new B(); 형태입니다. 시험에 자주 나옴.
...
//        animal2.fetch();  // 이게 불가능합니다
    /*
        ...
        클래스B 객체명 = new 클래스B();로 생성해버리면 재정의된 method로 호출되고 고유 method도 호출할 수 있으니까
    ...
    */
        ...
        // : 업캐스팅이 이루어지지 않고 그냥 Dog dog1 = new Dog();로 B b = new B(); 단 B = A의 자식 클래스인 경우
        ...
        // 저 위에 보시면 Animal animal2 = new Dog();로 생성했기 때문에 animal2의 Dog 버전 객체명이 아에 없었습니다.
...
//        dog3.makeSound();
        /*
            ...
            발생했음을 확인할 수 있습니다.
            즉 애초에 animal3를 생헝할 때 Animal클래스의 생성자를 호출해서 만들었기 때문에
            (A a = new A(); 이기 때문에)
            하위 클래스의 인스턴스로 다운캐스팅하는 것이 불가능합니다.
         */
        Animal animal4 = new Dog();
        // 와 같은 방식으로 선언된 클래스와 호출되는 새엇ㅇ자가 서로 달라야 합니다(그리고 둘은 상속관계여야만 합니다).
