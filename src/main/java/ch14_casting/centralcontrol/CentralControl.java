...
//        }
    ...
    // 비어있는 인덱스를 굳이 외부로 노출할 필요가 없기 대문에
    ...
    /*
        ...
        쓰는 경우가 많습니다. 하지만 0 과 너무 가까운 수이다 보니까 조직에 따라서 -100이라든지
    ...
    public void powerOn() {
        /*
            해당 클래스의 필드인 Power[] 배열 내에 있는 개체들은 기본적으로
        ...
        for(int i = 0; i < deviceArray.length; i++) {
            if(deviceArray[i] == null){
                ...
                continue;   // return; 으로 마무리 했을 때와는 콘솔 상에서의 차이점을 확인하면 더 명확해집니다.
                /*
                    ...
                    그러면 6번지는 비어있고 7번지는에 Power 의 서브 클래스의
...
// 향상된 for문으로 showInfo2() 작성
    ...
    public void deleteDevice(int deviceNumber) {
        int num = deviceNumber -1;
        for(int i = 0; i < deviceArray.length; i++) {
            if(deviceArray[num] == null) continue;
            if(deviceArray[num] != null) {
                System.out.println(deviceNumber + "번 째 기기가 제거되었습니다.");
                deviceArray[num] = null;
                return;
            }
            num = 0;
        }
    ...
    public void deleteDevice(String deviceName) {
        for (int i = 0; i < deviceArray.length; i++) {
            ...
            if(deviceArray[i].getClass().getSimpleName().equals(deviceName)) {
                System.out.println(deviceName + "이 제거되었습니다.");
                deviceArray[i] = null;
                return;
            }
        }
