...
public class Array03Method {
    // 이름만 받는 메서드
    private String[] inputNames() { // 이 메서드를 호출 했을 때 배열의 한계값도 입력하고 이름들도 입력하게끔 -> call3()
        ...
        scanner.nextLine();
        names = new String[num];                    // 여기서 변수를 초기화 해줬네요.
    ...
    public void writeNameAndScore() {
        String[] names = inputNames();          // inputNames() 메서드의 return형이 String 배열이니까 배열 선선해서 초기화
        double[] scores = inputScores(names);   // inputScores() 메서드는 String 배열을 argument로 요구하니까 포함하고,
                                                // return이 double 배열이니까 writheNameAndScore() 메서드 내에서
                                                // 실수 배열 선언 후 초기화

        System.out.println("학생들의 점수 총 합은 " + Arrays.stream(scores).sum() + "점 입니다.");
